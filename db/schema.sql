-- ============================================================
-- 개인 PC 사양 추천 시스템 — DB 스키마 (MySQL, DB명: shop_danawa)
-- 출처: https://github.com/neungjichai/Danawa-DB (New_crawler/danawa_only_load.sql
--       의 최종 products 테이블 + add_compat_columns.sql)의 테이블 구조를 그대로 가져왔다.
--       (원본은 DB명 DW_db를 썼으나, 이 프로젝트에서는 shop_danawa로 새로 생성한다.)
--
-- *** 원본과 다른 점 ***
-- 1) staging/unpivot/date_map 등 "가로형 CSV(날짜가 컬럼)" 적재용 중간 테이블은 뺐다.
--    이 프로젝트의 크롤러(shop.danawa.com 기반, danawa_crawler.py)는 "세로형 CSV
--    (crawl_date가 행)"를 만들기 때문에, 적재는 SQL이 아니라 db/load_from_csv.py
--    (Python)가 담당한다.
...
CREATE DATABASE IF NOT EXISTS shop_danawa CHARACTER SET utf8mb4;
USE shop_danawa;
